...
from drone3d.pytypes import OrientationQuaternion, PythonMsg
from drone3d.utils.load_utils import get_assets_file
from drone3d.visualization import glm
from drone3d.visualization.shaders import vtype as vertexobject_vtype
from drone3d.visualization.utils import join_vis
from drone3d.visualization.objects import OpenGLObject, UBOObject, VertexObject, \
    InstancedVertexObject, gl
            ...
            if node.mesh is not None:
                ...
                for primitive in mesh.primitives:
                    if get_bbox:
                        ...
                        if node.name in self.ignored_nodes:
                            return
        ...
        if self.size is not None:
            if self.size.root_tf is not None:
                ...
                if self.size.fixed_aspect:
                    scale = scale.min()
                    scale = [scale] * 3
                glm.scale(root_tf, *scale)

                # translate the object
                offset = (max_dims + min_dims) / 2 \
                    - (self.original_max_pos + self.original_min_pos) / 2 * scale
                if self.size.fixed_aspect and self.size.min_dims[2] == 0:
                    offset[2] = - self.original_min_pos[2] * scale[0]
...
def load_as_vertex_object(
        ...
        simple: bool = False)\
        -> VertexObject:
    ''' load a gltf object but as a single vertex object '''
    if color is None:
        color = [1., 0., 0., 1.]
    if ignored_nodes is None:
        ignored_nodes = []
    ...
    vis = []
    gltf.original_max_pos = np.array([-np.inf, -np.inf, -np.inf], dtype = np.float32)
    gltf.original_min_pos = np.array([ np.inf,  np.inf,  np.inf], dtype = np.float32)

    def _get_accessor(accessor_no: int):
        ''' load accessor data from self.buffers '''

        buffer_no, np_dtype, np_shape = get_accessor_info(accessor_no, gltf)
        data_slice = get_accessor_slice(accessor_no, gltf)
        if isinstance(data_slice, range):
            byte_data = buffers[buffer_no][data_slice.start: data_slice.stop]
        else:
            byte_data = bytes(buffers[buffer_no][k] for k in data_slice)
        data = np.frombuffer(byte_data, np_dtype).reshape(np_shape)
        return data

    def _process_node(node: Node, root_tf = None, get_bbox = False):
        # carry through transforms
        tf = get_node_tf(node)
        if root_tf is not None:
            tf = root_tf @ tf
        ...
        if node.mesh is not None:
            ...
            for primitive in mesh.primitives:
                if get_bbox:
                    verts = _get_accessor(primitive.attributes.POSITION)
                    verts = verts @ tf[:3,:3].T + tf[:3, -1]
                    gltf.original_max_pos = np.maximum(gltf.original_max_pos, verts.max(axis=0))
                    gltf.original_min_pos = np.minimum(gltf.original_min_pos, verts.min(axis=0))

                elif node.name not in ignored_nodes:
                    verts = _get_accessor(primitive.attributes.POSITION)
                    if primitive.attributes.NORMAL is not None:
                        norms = _get_accessor(primitive.attributes.NORMAL)
                    else:
                        norms = np.zeros(verts.shape)
                        norms[:,2] = 1
                    inds  = _get_accessor(primitive.indices)

                    V = np.zeros(verts.shape[0], dtype = vertexobject_vtype)
                    V['a_position'] = verts @ tf[:3,:3].T + tf[:3, -1]
                    V['a_normal'] = norms @ tf[:3,:3].T
                    V['a_color'] = color
        ...
        # loop over children
        for child in node.children:
            child_node = gltf.nodes[child]
            _process_node(child_node, root_tf = tf, get_bbox = get_bbox)

    root_tf = np.eye(4)
    # if a target size has been specified, scale for this
    if size is not None:
        if size.root_tf is not None:
            root_tf = size.root_tf
        else:
            # first get bounds of the entire object
            for node_num in gltf.scenes[0].nodes:
                node: Node = gltf.nodes[node_num]
                _process_node(node, root_tf, True)

            max_dims = np.array(size.max_dims)
            min_dims = np.array(size.min_dims)

            # scale the object, preserving the largest dimension if aspect ratio is fixed
            scale = (max_dims - min_dims)/(gltf.original_max_pos - gltf.original_min_pos)
            if size.fixed_aspect:
                scale = scale.min()
                scale = [scale] * 3
            glm.scale(root_tf, *scale)
            ...
            # translate the object
            offset = (max_dims + min_dims) / 2 \
                - (gltf.original_max_pos + gltf.original_min_pos) / 2 * scale
            if size.fixed_aspect and size.min_dims[2] == 0:
                offset[2] = - gltf.original_min_pos[2] * scale[0]

            M = np.eye(4)
            M[:3,-1] = np.array(offset)
            root_tf = M @ root_tf

            size.root_tf = root_tf

    # now add all the nodes
    for node_num in gltf.scenes[0].nodes:
        node: Node = gltf.nodes[node_num]
        _process_node(node, root_tf)

    V, I = join_vis(vis)
    if instanced:
        return InstancedVertexObject(ubo, V, I, simple=simple)
    return VertexObject(ubo, V, I, simple=simple)
...
def load_quadcopter(
    ...
    filename = get_assets_file('quadcopter.glb')
    return GLTFObject(filename, ubo, size, instanced=instanced)

def load_quadcopter_vertex_object(
        ubo: UBOObject,
        size: TargetObjectSize = None,
        instanced: bool = False,
        color: List[float] = None,
        simple: bool = False):
    ''' load quadcopter model as vertex object '''
    filename = get_assets_file('quadcopter.glb')
    return load_as_vertex_object(filename, ubo, size,
                                 instanced=instanced,
                                 color = color,
                                 simple = simple)
...
def _preview():
    ...
    size = TargetObjectSize(fixed_aspect = True, max_dims=[1,1,1],min_dims=[-1,-1,-1])

    quad = load_as_vertex_object(
        get_assets_file('quadcopter.glb'),
        window.ubo,
        size
    )
...
if __name__ == '__main__':
    #obj = GLTF2().load(get_assets_file('lambo2.glb'))
    #import pdb
    #pdb.set_trace()
    _preview()
