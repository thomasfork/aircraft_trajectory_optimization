''' obstacles and types for obstacles '''
from abc import abstractmethod, ABC
from dataclasses import dataclass, field

import numpy as np

from drone3d.pytypes import PythonMsg, Position, BodyPosition, OrientationQuaternion
...
class BasePolytopeObstacle(BaseObstacle):
    ...
    color: str = field(default = np.array([1,0,0,1]))
...
class RectangleObstacle(BasePolytopeObstacle):
    ...
    def __post_init__(self):
        if self.x is None:
            object.__setattr__(self, 'x', Position())
        if self.q is None:
            object.__setattr__(self, 'q', OrientationQuaternion())
        if self.w is None:
            object.__setattr__(self, 'w', BodyPosition(x1 = 1, x2 = 1, x3 = 1))
    ...
    def generate_texture(self, lines = False):
        ...
        Vertices['a_normal'] = (self.q.R() @ e).T
